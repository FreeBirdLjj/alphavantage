...
module AlphaVantage.Stock.TimeSeries.Monthly
  ( Monthly
  , monthlyMetadata
  , monthlyTimeSeries
  , getMonthly
...
instance Data.Aeson.FromJSON Monthly where
  parseJSON = Data.Aeson.withObject "Monthly" $ \o -> do
    metadata   <- o .: "Meta Data"
    timeSeries <- o .: "Monthly Time Series"
    return $ Monthly metadata timeSeries

monthlyMetadata :: Monthly -> MetaData
monthlyMetadata = metadata

monthlyTimeSeries :: Monthly -> Data.Map.Lazy.Map Data.Time.Calendar.Day TimePoint
monthlyTimeSeries = timeSeries
...
-- Required: apikey
--   Your API key.
getMonthly :: String -> String -> Network.HTTP.Client.Manager -> IO (Either String Monthly)
getMonthly equityName apiKey manager = do
  ...
  let request = Network.HTTP.Simple.setRequestMethod "GET"
              ...
              $ Network.HTTP.Simple.setRequestQueryString
                  [ ("function", Just "TIME_SERIES_MONTHLY")
  ...
  return (Data.Aeson.eitherDecode (Network.HTTP.Client.responseBody response) :: Either String Monthly)
