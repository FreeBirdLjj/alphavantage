...
module AlphaVantage.Stock.TimeSeries.DailyAdjusted
  ( DailyAdjusted
...
instance Data.Aeson.FromJSON TimePoint where
  parseJSON (Data.Aeson.Object v) = TimePoint
    ...
    <*> (fmap read $ v .: "8. split coefficient")

data DailyAdjusted = DailyAdjusted
  { metadata :: MetaData
  , timeSeries :: Data.Map.Lazy.Map Data.Time.Calendar.Day TimePoint
  } deriving (Eq, Read, Show)

instance Data.Aeson.FromJSON DailyAdjusted where
  parseJSON = Data.Aeson.withObject "DailyAdjusted" $ \o -> do
    metadata   <- o .: "Meta Data"
    timeSeries <- o .: "Time Series (Daily)"
    return $ DailyAdjusted metadata timeSeries

dailyAdjustedMetadata :: DailyAdjusted -> MetaData
dailyAdjustedMetadata = metadata

dailyAdjustedTimeSeries :: DailyAdjusted -> Data.Map.Lazy.Map Data.Time.Calendar.Day TimePoint
dailyAdjustedTimeSeries = timeSeries
...
-- Required: apikey
--   Your API key.
getDailyAdjusted :: String -> String -> Network.HTTP.Client.Manager -> IO (Either String DailyAdjusted)
getDailyAdjusted equityName apiKey manager = do
  ...
  let request = Network.HTTP.Simple.setRequestMethod "GET"
              ...
              $ Network.HTTP.Simple.setRequestQueryString
                  [ ("function", Just "TIME_SERIES_DAILY_ADJUSTED")
  ...
  return (Data.Aeson.eitherDecode (Network.HTTP.Client.responseBody response) :: Either String DailyAdjusted)
