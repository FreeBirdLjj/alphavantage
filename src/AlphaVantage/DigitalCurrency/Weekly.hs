...
module AlphaVantage.DigitalCurrency.Weekly
  ( Weekly
  , weeklyMetadata
  , weeklyTimeSeries
  , getWeekly
...
instance Data.Aeson.FromJSON Weekly where
  parseJSON (Data.Aeson.Object v) = Weekly
    <$> v .: "Meta Data"
    <*> v .: "Time Series (Digital Currency Weekly)"
...
-- Required: apikey
--   Your API key.
getWeekly :: String -> String -> String -> Network.HTTP.Client.Manager -> IO (Either String Weekly)
getWeekly digitalCurrencyName marketName apiKey manager = do
  ...
  let request = Network.HTTP.Simple.setRequestMethod "GET"
              ...
              $ Network.HTTP.Simple.setRequestQueryString
                  [ ("function", Just "DIGITAL_CURRENCY_WEEKLY")
