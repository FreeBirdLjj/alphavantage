...
module AlphaVantage.DigitalCurrency.Monthly
  ( Monthly
  , monthlyMetadata
  , monthlyTimeSeries
  , getMonthly
...
instance Data.Aeson.FromJSON Monthly where
  parseJSON (Data.Aeson.Object v) = Monthly
    <$> v .: "Meta Data"
    <*> v .: "Time Series (Digital Currency Monthly)"
...
-- Required: apikey
--   Your API key.
getMonthly :: String -> String -> String -> Network.HTTP.Client.Manager -> IO (Either String Monthly)
getMonthly digitalCurrencyName marketName apiKey manager = do
  ...
  let request = Network.HTTP.Simple.setRequestMethod "GET"
              ...
              $ Network.HTTP.Simple.setRequestQueryString
                  [ ("function", Just "DIGITAL_CURRENCY_MONTHLY")
